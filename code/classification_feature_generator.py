...
for i in range(rows):
    for j in range(7):
        ...
        if i >= val[j] and df.loc[i - val[j] ,'Search_Rate']!=0:  #Proportion of change in search rate
            df.loc[i,'PS'+chr(48+j)]= (df.loc[i,'Search_Rate']-df.loc[i - val[j],'Search_Rate'])/(df.loc[i - val[j] ,'Search_Rate']) 
        elif i < val[j] and df.loc[0 ,'Search_Rate']!=0 :
            df.loc[i,'PS'+chr(48+j)]=(df.loc[i,'Search_Rate']-df.loc[0,'Search_Rate'])/(df.loc[0 ,'Search_Rate'])  
        else :
            df.loc[i,'PS'+chr(48+j)]=100
        if i >= val[j] and df.loc[i  ,'Search_Rate']!=0:  #max search rate over interval
            df.loc[i,'MaxS'+chr(48+j)]= (df.loc[i - val[j] : i,'Search_Rate'].max())/(df.loc[i ,'Search_Rate']) 
        elif i < val[j] and df.loc[i ,'Search_Rate']!=0 :
            df.loc[i,'MaxS'+chr(48+j)]=(df.loc[0 : i,'Search_Rate'].max())/(df.loc[i ,'Search_Rate']) 
        else :
            df.loc[i,'MaxS'+chr(48+j)]=100
        if i >= val[j] and df.loc[i  ,'Search_Rate']!=0:  #ratio of Avg Search Rate over interval
            df.loc[i,'AvS'+chr(48+j)]= (df.loc[i - val[j] : i,'Search_Rate'].mean())/(df.loc[i ,'Search_Rate']) 
        elif i < val[j] and df.loc[i ,'Search_Rate']!=0  :
            df.loc[i,'AvS'+chr(48+j)]= (df.loc[0: i,'Search_Rate'].mean())/(df.loc[i ,'Search_Rate']) 
        else :
            df.loc[i,'AvS'+chr(48+j)]=100
...
df.to_csv('nsfdata.csv',sep=",",index=False)